...
PNG_DIMENSIONS = {
    "wine_logo.png": (240, 72, 11, 14, 20, 255),
    "ic_launcher_foreground.png": (432, 432, 11, 14, 20, 255),
    "ic_launcher.png": (512, 512, 11, 14, 20, 255),
    "features_auth_ic_logo_landscape.png": (96, 96, 11, 14, 20, 255),
    "features_auth_ic_logo_overseas.png": (366, 72, 11, 14, 20, 255),
    "splash_logo.png": (996, 200, 11, 14, 20, 255),
    "bh_explore_gog.png": (512, 512, 11, 14, 20, 255),
    "bh_explore_logo.png": (512, 512, 11, 14, 20, 255),
    "bannerhub-v6-logo.png": (512, 512, 11, 14, 20, 255),
}
...
def render_artwork_png(filename, target_path):
    ...
    deck_candidates = [
        os.path.join(REPO, "src", "assets", "images", "deskhub_v108_icon_1787518506267.jpg"),
        os.path.join(REPO, "src", "assets", "images", "deskhub_logo_1787433177158.jpg"),
        os.path.join(REPO, "Deck.jpg"),
    ...
    if shutil.which("convert") and deck_src:
        try:
            if filename == "ic_launcher_foreground.png":
                cmd = f"convert '{deck_src}' -resize 432x432 -gravity center '{target_path}'"
                subprocess.check_call(cmd, shell=True)
                return True
            elif filename in ["ic_launcher.png", "bh_explore_logo.png", "bh_explore_gog.png", "bannerhub-v6-logo.png"]:
                cmd = f"convert '{deck_src}' -resize 512x512 -gravity center '{target_path}'"
                subprocess.check_call(cmd, shell=True)
                return True
            elif filename == "wine_logo.png":
                cmd = f"convert '{deck_src}' -resize 240x72 -gravity center '{target_path}'"
                subprocess.check_call(cmd, shell=True)
                return True
            elif filename == "features_auth_ic_logo_landscape.png":
                cmd = f"convert '{deck_src}' -resize 96x96 -gravity center '{target_path}'"
                subprocess.check_call(cmd, shell=True)
                return True
            elif filename == "features_auth_ic_logo_overseas.png":
                cmd = f"convert '{deck_src}' -resize 366x72 -gravity center '{target_path}'"
                subprocess.check_call(cmd, shell=True)
                return True
            elif filename == "splash_logo.png":
                cmd = f"convert '{deck_src}' -resize 996x200 -gravity center '{target_path}'"
                subprocess.check_call(cmd, shell=True)
                return True
        except Exception as e:
            print(f"[WARN] convert from deck failed: {e}")

    dim = PNG_DIMENSIONS.get(filename, (512, 512, 11, 14, 20, 255))
